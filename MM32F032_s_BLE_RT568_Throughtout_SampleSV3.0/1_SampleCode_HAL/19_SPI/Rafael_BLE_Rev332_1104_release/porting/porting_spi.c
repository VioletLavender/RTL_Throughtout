...
void BSP_SPI_BLE_Init(void)
{
    ...
    /* initial SPI PDMA */
//    SPI_PDMA_Init();
    //DMA channel 14, 15 not enable INT_TC
...
void SPI_1BYT_SetTx(uint8_t regAddr, uint8_t u8SrcData)
{
    ...
    while (!(SPI2->SR & SPI_FLAG_TXEPT)) //must wait for TX data send out, then NSS high
    {
    }
...
/*
    ...
    //check SPI DMA transfer complete status
    while(1)
    {
      u32i = DMA1->ISR & (DMA1_FLAG_TC4|DMA1_FLAG_TC5);
      if(u32i)  
      {
         DMA1->IFCR = u32i;  //clear DMA transfer complete flag
      }
      SPI2->NSSR |= SPI_NSSR_NSS ; //CS HIGH
      break;
    }
*/
...
//    SPI0_ClearRxFIFO();
    ...
    //SPIx_WriteByte(SPI2, (regAddr&0x80)>>7);
...
//    }
    ...
    SPIx_DMA_RxData(SPI2,(uint8_t *)u32DstAddr,u32TransCount); 
...
void SPI_PDMA_SetTx(uint8_t regAddr, uint32_t u32SrcAddr, uint32_t u32TransCount)
{
    ...
    SPIx_DMA_TxData(SPI2,(uint8_t *)u32SrcAddr,u32TransCount); 
